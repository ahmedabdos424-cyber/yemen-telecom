-- Rollback for migration 025 (enable_rls): remove RLS from all public tables.
-- Run ONLY to revert. Restores pre-RLS behaviour where the public anon/
-- authenticated Supabase roles again have direct table access.
-- NOTE: lives OUTSIDE server/migrations/ so it is never auto-applied by the
-- deploy pipeline. Run manually via the Supabase SQL editor or supabase_execute_sql.
